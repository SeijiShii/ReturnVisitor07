...
import work.ckogyo.returnvisitor.utils.toJPDateText
import java.lang.Exception
import java.util.*
import kotlin.collections.ArrayList

class MonthlyReport (val month: Calendar, private val weekStart: WeekStart) {
...
//            Log.d(debugTag, "first: ${first.toJPDateText()}")

            val last = month.clone() as Calendar
            last.set(Calendar.DAY_OF_MONTH, 1)
            last.add(Calendar.MONTH, 1)
            last.add(Calendar.DAY_OF_MONTH, -1)
//            Log.d(debugTag, "last: ${last.toJPDateText()}")
